...
public class ComputeMileage extends ConsoleProgram {
	 ...
	 * index corresponding to the city, if it exits. If the city name is 
	...
	private int linearSearch(String key, String[] array) {
		...
		for (int i = 0; i < array.length; i++) {
			System.out.println("number of times through here " + i);
			if (key.toLowerCase().equals(array[i].toLowerCase())) return i;
		}
		return -1;
	}
	...
	private int binarySearch(String key, String[] array) {
		...
		int counter = 0;
		while (lh <= rh) {
			System.out.println("times in while loop " + counter);
			counter += 1;
			int mid = (lh + rh) / 2;
